...
int RCCE_APP(int argc, char **argv){
  ...
  ID = RCCE_ue();
  if (NP<2) {
    if (ID==0) printf("Need at least two cores to run thisa code\n");
    return(1);
  }
  ...
  bufsize = atoi(*++argv);
  if ((bufsize<1)) {
    if (ID==0) printf("Buffer size must be positive multiple of 4\n");
    return(1);
  }
  ...
  for (round=0; round<nrounds; round++) {
    ...
    if (ID==(round+1)%NP) {
      ...
      if (round==nrounds-1) {
        ...
        printf("Final sum on UE %03d equals %f, refval = %f\n", ID, sum, refval);
      }
    }

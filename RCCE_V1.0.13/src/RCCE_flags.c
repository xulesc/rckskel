...
#endif
  RC_cache_invalidate();
  return (RCCE_bit_value(flag.line_address, flag.location) == RCCE_FLAG_SET);
}
...
#endif
  RC_cache_invalidate();
  // read first word on MPB line, which contains flag value; rest is unused
  return ((*flag) == RCCE_FLAG_SET);
}
...
//--------------------------------------------------------------------------------------
int    RCCE_flag_alloc(RCCE_FLAG *flag) {
  *flag = (int *) RCCE_malloc(RCCE_LINE_SIZE);
